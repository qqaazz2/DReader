...
class ImageNode extends InlineNode {
  ...
  late ui.Image image;
  String imgName;
  double imageHeight = 0, imageWidth = 0, top = 0, left = 0;

  ImageNode(super.tag, super.styleModel, super.uniqueId, this.bytes, this.imgName);
  ...
  List<ReaderNode> layout(double availableWidth, Offset offset, {isFull}) {
    ...
    if (hasExplicitSize) {
      ...
      if (currentWidth > availableWidth) {
        ...
        left = (availableWidth - currentWidth) / 2;
      }

      if (currentHeight < NodeStatus.pageHeight) {
        top = (NodeStatus.pageHeight - currentHeight) / 2;
      }

      currentOffset = Offset(currentOffset.dx, currentOffset.dy);
      imageWidth = currentWidth;
      imageHeight = currentHeight;
      currentHeight += top * 2;
      currentWidth += left * 2;
  ...
  void paint(ui.Canvas canvas, ui.Offset offset) {
    ...
    Rect dst = Rect.fromLTWH(currentOffset.dx + left, currentOffset.dy + top,
